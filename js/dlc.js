...
function dlcUrl() {
    return 'http://tzshg.app.xiaozhuschool.com';
}

function redirect() {
    return location.href = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx42a447a217e4de78&redirect_uri=' + location.href + '&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
}
...
function dlc_request1(method, data, cb, type) {
    var data = data || {};
    data.token = "oKYxK0jkShSOVDEk5RGVkErI_-_Y"
    url = dlcUrl() + method;
    $.ajax({
        type: type ? type : 'post',
        url: url,
